...
def init_run(cfg: DictConfig, name: str):
    ...
    try:
        _run = wandb.init(project=cfg.wandb.project, entity=cfg.wandb.get('entity', None) or None, name=name, config=OmegaConf.to_container(cfg, resolve=True), reinit=True, mode=os.environ.get('WANDB_MODE', 'offline'))
    except Exception:
        _run = None
    return _run
