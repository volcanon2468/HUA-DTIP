...
def pearson_r(pred: torch.Tensor, target: torch.Tensor) -> float:
    ...
    return float(cov / (std_p * std_t + 1e-8))
...
def activity_f1(logits: torch.Tensor, true_labels: torch.Tensor) -> float:
    preds = logits.argmax(dim=-1).cpu().numpy()
    truth = true_labels.cpu().numpy()
    return float(f1_score(truth, preds, average='macro', zero_division=0))
