...
class ADWINDetector:
    ...
    def update(self, feature_vec: np.ndarray, timestamp: float=None) -> dict:
        ...
        for i, val in enumerate(feature_vec):
            ...
            elif self.detectors[i].update(float(val)):
                drifted_features.append(i)
        result = {'drift_detected': len(drifted_features) > 0, 'drifted_features': drifted_features, 'n_drifted': len(drifted_features), 'timestamp': timestamp}
