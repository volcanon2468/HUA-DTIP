...
class DriftDiagnoser:
    FEATURE_GROUPS = {
        "imu_gait":    list(range(0, 8)),
        "imu_stats":   list(range(8, 20)),
        "cardio_hr":   list(range(20, 30)),
        "cardio_hrv":  list(range(30, 40)),
        "quality":     list(range(40, 48)),
    }
    ...
    def diagnose(self, adwin_result: dict, mmd_result: dict, ae_result: dict) -> dict:
        ...
        if ae_result.get("drift_detected"):
            causes.append({"type": "novelty", "error": ae_result.get("reconstruction_error", 0.0),
                          "ratio": ae_result.get("novelty_ratio", 0.0)})
        ...
        return {
            ...
            "n_detectors_triggered": sum([
                adwin_result.get("drift_detected", False),
                mmd_result.get("drift_detected", False),
                ae_result.get("drift_detected", False),
            ]),
        }
