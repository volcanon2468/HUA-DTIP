...
class FedProxServer:
    ...
    def aggregate(self, client_states: List[OrderedDict], client_weights: List[float]=None):
        ...
        for key in self.global_state.keys():
            if key in client_states[0]:
                new_state[key] = sum((client_weights[i] * client_states[i][key].float() for i in range(n)))
            else:
    ...
    def get_proximal_term(self, local_model: nn.Module) -> torch.Tensor:
        ...
        for (name, local_p), (_, global_p) in zip(local_model.named_parameters(), self.global_model.named_parameters()):
