...
class FedPerClient:
    ...
    def __init__(self, model: nn.Module, personal_layer_names: List[str], client_id: int, lr: float=0.001, local_epochs: int=5):
        self.model = model
    ...
    def _save_personal_layers(self):
        ...
        self.personal_state = {k: v.clone() for k, v in state.items() if any((pl in k for pl in self.personal_layer_names))}
    ...
    def receive_global(self, global_state: OrderedDict):
        filtered = {k: v for k, v in global_state.items() if not any((pl in k for pl in self.personal_layer_names))}
