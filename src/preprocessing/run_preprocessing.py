...
from src.preprocessing.dataset_loaders import MHEALTHDataset, PAMAP2Dataset, FourWeekPPGDataset, IMU_COLS, ECG_COLS, PAMAP2_IMU_COLS
...
def preprocess_mhealth(cfg: DictConfig, out_dir: str):
    ...
    for df in ds.get_all_dfs():
        ...
        stats = process_subject(imu_filt, ecg_filt, _feat_fn, _hrv_fn, subject_id=sid, out_dir=out_dir, window_size=cfg.window.size, stride=cfg.window.stride, label_seq=labels)
...
def preprocess_pamap2(cfg: DictConfig, out_dir: str):
    ...
    for df in ds.get_all_dfs():
        ...
        stats = process_subject(imu_filt, cardio_raw, _feat_fn, _hrv_fn, subject_id=sid, out_dir=out_dir, window_size=cfg.window.size, stride=cfg.window.stride)
...
def preprocess_ppg_4week(cfg: DictConfig, out_dir: str):
    ...
    for sid, days in ds.records.items():
        for day_idx, (day_key, df) in enumerate(sorted(days.items())):
            ...
            process_subject(imu, ppg[:, None], _feat_fn, _hrv_fn, subject_id=sid, out_dir=out_dir, window_size=cfg.window.size, stride=cfg.window.stride)
