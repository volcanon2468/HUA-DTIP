...
def extract_imu_features(imu: np.ndarray) -> np.ndarray:
    ...
    cadence = zero_crosses / (len(vert) / 50.0)
    ...
    features = np.nan_to_num(np.array([cadence, step_count, step_regularity, sma, mad, energy, stride_length, gait_symmetry, *acc_mean, *acc_std, *gyro_mean, *gyro_std], dtype=np.float32))
    assert len(features) == 20, f'Expected 20 IMU features, got {len(features)}'
    return features
...
def extract_cardio_features(cardio: np.ndarray, hr: float=None, fs: float=50.0) -> np.ndarray:
    ...
    if len(peaks) >= 2:
        ...
        hr_std = float(60.0 * rr.std() / (rr.mean() ** 2 + 1e-08))
    ...
    lf_hf_ratio = lf_power / (hf_power + 1e-08)
    hr_max_est = 220.0
    hr_reserve_pct = max(0.0, (hr_mean - 60.0) / (hr_max_est - 60.0 + 1e-08))
    ...
    parasympathetic_index = hf_power / (lf_power + hf_power + 1e-08)
    sympathetic_index = lf_power / (lf_power + hf_power + 1e-08)
    features = np.array([hr_mean, hr_std, hr_min, hr_max, hr_range, rmssd, sdnn, lf_power, hf_power, lf_hf_ratio, hr_reserve_pct, recovery_rate_proxy, ppg_amplitude, ppg_rise_time, ppg_fall_time, resp_rate_proxy, spo2_proxy, cardiac_output_proxy, parasympathetic_index, sympathetic_index], dtype=np.float32)
    assert len(features) == 20, f'Expected 20 cardio features, got {len(features)}'
    return features
...
def extract_quality_features(imu: np.ndarray, cardio: np.ndarray) -> np.ndarray:
    ...
    artifact = float(power[freqs > 10].sum() / (power.sum() + 1e-08))
    wander = float(power[freqs < 0.5].sum() / (power.sum() + 1e-08))
    sig = cardio[:, 0]
    dsig = np.diff(sig)
    peaks = np.where((dsig[:-1] > 0) & (dsig[1:] <= 0))[0]
    ...
    return np.array([imu_snr, cardio_snr, ppg_snr, completeness, artifact, wander, rr_count, beat_quality], dtype=np.float32)
...
def extract_all_features(imu: np.ndarray, cardio: np.ndarray, hr: float=None) -> np.ndarray:
    ...
    assert len(full) == 48, f'Expected 48 total features, got {len(full)}'
    return full