...
class SlidingWindowCNNBlock(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x = x.permute(0, 2, 1)   # (B, 9, T)
        out5  = self.pool(self.branch_5(x)).squeeze(-1)    # (B, 85)
        out7  = self.pool(self.branch_7(x)).squeeze(-1)    # (B, 85)
        out11 = self.pool(self.branch_11(x)).squeeze(-1)   # (B, 86)
...
class ChannelTimeAttentionTransformer(nn.Module):
    ...
    def forward(self, h: torch.Tensor) -> torch.Tensor:
        h = h.unsqueeze(1)              # (B, 1, 256)
        h = self.transformer(h)         # (B, 1, 256)
