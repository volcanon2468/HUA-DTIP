...
class SlidingWindowCNNBlock(nn.Module):
    ...
    def _conv_block(self, in_ch, out_ch, kernel_size):
        return nn.Sequential(nn.Conv1d(in_ch, 64, kernel_size=kernel_size, padding=kernel_size // 2), nn.BatchNorm1d(64), nn.ReLU(inplace=True), nn.Conv1d(64, 128, kernel_size=kernel_size, padding=kernel_size // 2), nn.BatchNorm1d(128), nn.ReLU(inplace=True), nn.Conv1d(128, out_ch, kernel_size=kernel_size, padding=kernel_size // 2), nn.BatchNorm1d(out_ch), nn.ReLU(inplace=True))
...
class SWCTNet(nn.Module):
    ...
    def __init__(self, in_channels: int=9, output_dim: int=256, n_heads: int=8, n_layers: int=3, dropout: float=0.1):
        super().__init__()
    ...
    def classify(self, x: torch.Tensor) -> torch.Tensor:
        h = self.forward(x)
        assert self.classifier is not None, 'Call build_classifier(n_classes) before classify().'
