...
class CrossModalFusion(nn.Module):
    def __init__(self, common_dim: int = 128, n_heads: int = 4,
                 n_layers: int = 2, ff_dim: int = 256, output_dim: int = 128):
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=common_dim, nhead=n_heads,
    ...
    def forward(self,
                h_imu: torch.Tensor,
                h_cardio: torch.Tensor,
                h_feat: torch.Tensor) -> torch.Tensor:
