...
class TCNBlock(nn.Module):
    def __init__(self, in_channels: int, out_channels: int,
                 kernel_size: int = 3, dilations: list = None, dropout: float = 0.1):
        super().__init__()
        ...
        self.blocks = nn.ModuleList([
            DilatedResidualBlock(out_channels, kernel_size, d, dropout) for d in dilations
        ])
...
class MicroScaleModel(nn.Module):
    def __init__(self, input_dim: int = 128, tcn_channels: int = 256,
                 tcn_kernel: int = 3, dilations: list = None,
                 transformer_heads: int = 8, transformer_layers: int = 3,
                 transformer_dim: int = 256, ff_dim: int = 512,
                 sequence_len: int = 180, output_dim: int = 256, dropout: float = 0.1):
        super().__init__()
        dilations = dilations or [1, 2, 4, 8]
