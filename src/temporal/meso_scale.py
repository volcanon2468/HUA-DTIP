...
class MesoScaleModel(nn.Module):
    ...
    def __init__(self, input_dim: int=512, transformer_heads: int=8, transformer_layers: int=4, ff_dim: int=1024, sequence_len: int=7, output_dim: int=512, dropout: float=0.1):
        super().__init__()
        self.dow_embed = nn.Embedding(7, input_dim)
        self.pos_embed = nn.Embedding(sequence_len, input_dim)
        self.input_norm = nn.LayerNorm(input_dim)
        encoder_layer = nn.TransformerEncoderLayer(d_model=input_dim, nhead=transformer_heads, dim_feedforward=ff_dim, dropout=dropout, batch_first=True, norm_first=True)
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=transformer_layers)
        self.norm = nn.LayerNorm(input_dim)
        recency = [0.05, 0.07, 0.1, 0.12, 0.16, 0.2, 0.3]
