...
class HierarchicalFusion(nn.Module):
    ...
    def __init__(self, micro_dim: int=256, meso_dim: int=512, macro_dim: int=128, proj_dim: int=256, output_dim: int=512):
        super().__init__()
        ...
        self.gate = nn.Sequential(nn.Linear(proj_dim * 3, proj_dim * 3), nn.Sigmoid())
        self.output_proj = nn.Sequential(nn.Linear(proj_dim * 3, output_dim), nn.LayerNorm(output_dim), nn.ReLU(inplace=True))
    ...
    def forward(self, z_micro: torch.Tensor, z_meso: torch.Tensor, z_macro: torch.Tensor) -> torch.Tensor:
        ...
        meso_ctx, _ = self.macro_to_meso_attn(p_meso.unsqueeze(1), p_macro.unsqueeze(1), p_macro.unsqueeze(1))
        meso_ctx = p_meso + meso_ctx.squeeze(1)
        micro_ctx, _ = self.meso_to_micro_attn(p_micro.unsqueeze(1), meso_ctx.unsqueeze(1), meso_ctx.unsqueeze(1))
