...
class TwinDecoder(nn.Module):
    def __init__(self, latent_dim: int = 10, output_dim: int = 512):
        super().__init__()
        self.net = nn.Sequential(
...
class PredictionHead(nn.Module):
    def __init__(self, latent_dim: int = 10, activity_dim: int = 64, output_dim: int = 6):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(latent_dim + activity_dim, 64),
            nn.ReLU(inplace=True),
...
class BayesianVAE(nn.Module):
    def __init__(
        ...
        self.decoder = TwinDecoder(latent_dim, input_dim)
    ...
    def forward(self, z_temporal: torch.Tensor):
        ...
        activity_ctx = self.activity_proj(recon)
    ...
    def loss(
        ...
        hr_true: torch.Tensor,
        hrv_true: torch.Tensor,
    ):
        z, mu, logvar, recon, pred = self.forward(z_temporal)

        l_recon = F.mse_loss(recon, z_temporal)

        l_kl = -0.5 * torch.mean(1.0 + logvar - mu.pow(2) - logvar.exp())

        l_pred = F.mse_loss(pred[:, 0], hr_true) + F.mse_loss(pred[:, 1:6], hrv_true)

        total = l_recon + self.beta * l_kl + 0.0001 * l_pred
        return total, {
            'recon': l_recon.item(),
            'kl': l_kl.item(),
            'pred': l_pred.item(),
