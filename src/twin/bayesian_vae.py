...
class BayesianVAE(nn.Module):
    def __init__(self, input_dim: int = 512, latent_dim: int = 32,
                 decoder_out_dim: int = 256, beta: float = 0.01):
        super().__init__()
        ...
        self.recon_proj = nn.Linear(decoder_out_dim, 48)  # reconstruct only 48 features
    ...
    def loss(self, z_temporal: torch.Tensor, hr_true: torch.Tensor, hrv_true: torch.Tensor):
        ...
        target_recon = z_temporal[:, :48]  # only the 48 actual features
        ...
        return l_recon + self.beta * l_kl + 1e-4 * l_pred, {
            "recon": l_recon.item(),
            "kl":    l_kl.item(),
            "pred":  l_pred.item(),
        }
