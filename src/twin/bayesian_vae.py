...
class TwinEncoder(nn.Module):
    def __init__(self, input_dim: int = 512, latent_dim: int = 10, dropout: float = 0.2):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(input_dim, 256), nn.LayerNorm(256), nn.ReLU(inplace=True), nn.Dropout(dropout),
            nn.Linear(256, 128),       nn.LayerNorm(128), nn.ReLU(inplace=True), nn.Dropout(dropout),
        )
...
class TwinDecoder(nn.Module):
    def __init__(self, latent_dim: int = 10, output_dim: int = 256):
        super().__init__()
        self.net = nn.Sequential(
...
class PredictionHead(nn.Module):
    def __init__(self, latent_dim: int = 10, activity_dim: int = 64, output_dim: int = 6):
        super().__init__()
        self.net = nn.Sequential(
            nn.Linear(latent_dim + activity_dim, 64), nn.ReLU(inplace=True),
...
class BayesianVAE(nn.Module):
    def __init__(self, input_dim: int = 512, latent_dim: int = 10,
                 decoder_out_dim: int = 256, beta: float = 4.0):
        super().__init__()
    ...
    def loss(self, z_temporal: torch.Tensor, hr_true: torch.Tensor, hrv_true: torch.Tensor):
        ...
        target_recon = z_temporal[:, :256] if z_temporal.shape[-1] >= 256 else z_temporal
        ...
        return l_recon + self.beta * l_kl + l_pred, {
            "recon": l_recon.item(),
            "kl":    l_kl.item(),
            "pred":  l_pred.item(),
        }
