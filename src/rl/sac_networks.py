...
class TwinGymEnv(gym.Env):
    ...
    def step(self, action: np.ndarray):
        ...
        info = {
            'z_mu' : self.z_mu.cpu().numpy(),
            'z_std': self.z_std.cpu().numpy(),
            'day'  : self.t,
        }
    ...
    def _compute_reward(self, action: np.ndarray) -> float:
        ...
        r_progress  = 0.30 * (capacity + cardio)
        ...
        r_recovery  = 0.20 * recovery
        r_stability = 0.10 * stability
...
class SquashedGaussianActor(nn.Module):
    ...
    def sample(self, state: torch.Tensor):
        ...
        x_t  = dist.rsample()
        tanh_action = torch.tanh(x_t)

        log_prob = dist.log_prob(x_t) - torch.log(1.0 - tanh_action.pow(2) + 1e-6)
        log_prob = log_prob.sum(dim=-1, keepdim=True)

        action = (tanh_action + 1.0) / 2.0
