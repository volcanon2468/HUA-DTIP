...
class TwinGymEnv(gym.Env):
    ...
    def __init__(self, vae: BayesianVAE, sde: LatentNeuralSDE, episode_len: int=28, device: str='cpu'):
        super().__init__()
        self.vae = vae
        self.sde = sde
        ...
        self.observation_space = spaces.Box(low=-5.0, high=5.0, shape=(self.latent_dim * 2,), dtype=np.float32)
        self.action_space = spaces.Box(low=np.zeros(6, dtype=np.float32), high=np.ones(6, dtype=np.float32))
        self.t = 0
        self.z_mu = None
        self.z_std = None
...
class SquashedGaussianActor(nn.Module):
    ...
    def __init__(self, state_dim: int=64, action_dim: int=6, hidden: int=256):
        super().__init__()
        self.net = nn.Sequential(nn.Linear(state_dim, hidden), nn.ReLU(inplace=True), nn.Linear(hidden, hidden), nn.ReLU(inplace=True))
    ...
    def sample(self, state: torch.Tensor):
        ...
        log_prob = dist.log_prob(x_t) - torch.log(1 - action.pow(2) + 1e-06)
...
class TwinCritic(nn.Module):
    ...
    def __init__(self, state_dim: int=64, action_dim: int=6, hidden: int=256):
        super().__init__()
        self.q1 = nn.Sequential(nn.Linear(state_dim + action_dim, hidden), nn.ReLU(inplace=True), nn.Linear(hidden, hidden), nn.ReLU(inplace=True), nn.Linear(hidden, 1))
        self.q2 = nn.Sequential(nn.Linear(state_dim + action_dim, hidden), nn.ReLU(inplace=True), nn.Linear(hidden, hidden), nn.ReLU(inplace=True), nn.Linear(hidden, 1))
