...
class MultiObjectiveReward:
    def __init__(self, w_progress: float = 0.35, w_safety: float = 0.30,
                 w_recovery: float = 0.20, w_adherence: float = 0.15):
        self.w_progress  = w_progress
        self.w_safety    = w_safety
        self.w_recovery  = w_recovery
        self.w_adherence = w_adherence

    def compute(self, z_mu: np.ndarray, z_std: np.ndarray, action: np.ndarray,
                prev_z_mu: np.ndarray = None) -> float:
        ...
        total = (self.w_progress  * r_progress +
    ...
    def decompose(self, z_mu: np.ndarray, z_std: np.ndarray, action: np.ndarray) -> dict:
        ...
        return {
            "progress":  capacity + 0.5 * cardio,
            "safety":    -max(0.0, fatigue - 1.0) * 2.0 - max(0.0, z_std.mean() - 1.0),
            "recovery":  recovery * (1.0 - action[0]) + action[2] * 0.5,
            "adherence": (1.0 - abs(action[0] - 0.5)) + stability * 0.5,
        }
