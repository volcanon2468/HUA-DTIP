...
def _fitness_score(result: RolloutResult, plan: InterventionPlan) -> float:
    hr_improvement = max(0.0, (result.hr_trajectory[0] - result.hr_trajectory[-1]) / 20.0)
...
class WhatIfEngine:
    ...
    def query(self, z0: torch.Tensor, plan_kwargs: dict, n_days: int=28) -> ScenarioResult:
        ...
        return ScenarioResult(name=f'intensity={plan.intensity:.2f}_dur={plan.duration_days}d', plan=plan_kwargs, overtraining_prob=result.overtraining_prob, injury_prob=result.injury_prob, peaking_day=result.peaking_day, avg_hr=float(result.hr_trajectory.mean()), avg_hrv_sdnn=float(result.hrv_trajectory[:, 0].mean()), max_risk=float(result.risk_scores.max()), fitness_score=_fitness_score(result, plan))
    ...
    def grid_search(self, z0: torch.Tensor, intensities: list=None, durations: list=None, n_days: int=28) -> List[ScenarioResult]:
        intensities = intensities or [0.3, 0.5, 0.7, 0.9]
        durations = durations or [7, 14, 21]
        scenarios = []
        for intensity in intensities:
            for dur in durations:
                scenarios.append({'name': f'I{intensity:.1f}_D{dur}', 'intensity': intensity, 'duration_days': dur})
