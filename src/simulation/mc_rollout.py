...
class InterventionPlan:
    ...
    def to_activity_tensor(self, device="cpu") -> torch.Tensor:
        return torch.tensor([
            self.intensity,
            self.duration_days / 28.0,
            self.rest_extra_hours / 8.0,
            self.nutrition_quality,
            self.sleep_consistency,
            min(1.0, self.intensity * self.duration_days / 14.0),
...
class MCRolloutEngine:
    ...
    @torch.no_grad()
    def rollout(self, z0_mean: torch.Tensor, z0_std: torch.Tensor,
                plan: InterventionPlan, n_days: int = 28) -> RolloutResult:
        ...
        z0_samples = z0_mean.unsqueeze(0) + z0_std.unsqueeze(0) * torch.randn(
            self.n_samples, z0_mean.shape[-1], device=self.device
        )
        ...
        overtraining_prob = float(np.mean(
            all_trajs[:, :, 0].numpy() < -2.0
        ))
        injury_prob = float(np.mean(
            all_trajs[:, :, 1].numpy() > 2.0
        ))
        peaking_day = int(np.argmax(hr_traj)) if len(hr_traj) > 0 else -1

        return RolloutResult(
            z_mean=z_mean, z_std=z_std,
            z_trajectories=all_trajs.numpy(),
            hr_trajectory=hr_traj,
            hrv_trajectory=hrv_traj,
            risk_scores=risk_scores,
            overtraining_prob=overtraining_prob,
            injury_prob=injury_prob,
            peaking_day=peaking_day,
        )
