...
TARGET_METRICS = {'imu_loso_f1': {'target': 0.75, 'direction': 'higher_better'}, 'hr_mae_bpm': {'target': 35.0, 'direction': 'lower_better'}, 'reconstruction_mse': {'target': 0.05, 'direction': 'lower_better'}, 'trajectory_mae': {'target': 0.12, 'direction': 'lower_better'}, 'uncertainty_calibration': {'target': 0.9, 'direction': 'higher_better'}, 'sac_mean_reward': {'target': -20.0, 'direction': 'higher_better'}, 'improvement_over_random': {'target': -5.0, 'direction': 'higher_better'}, 'avg_personalization_improvement_pct': {'target': 15.0, 'direction': 'higher_better'}}
...
def generate_report() -> dict:
    ...
    report = {'project': 'HUA-DTIP', 'timestamp': time.strftime('%Y-%m-%d %H:%M:%S'), 'device': str(device)}
    print('\n' + '=' * 60)
    ...
    try:
        ...
        report['federated'] = {k: v for k, v in fed.items() if k not in ('per_subject_personalization', 'per_subject_cold_start')}
    except Exception as e:
    ...
    for metric, value in all_metrics.items():
        ...
        if result != 'no_target':
            scorecard[metric] = {'value': value, 'target': TARGET_METRICS.get(metric, {}).get('target'), 'result': result}
