...
def evaluate_policy(actor, env, safety, reward_fn, n_eval: int=50, deterministic: bool=True):
    ...
    return {'mean_reward': float(np.mean(ep_rewards)), 'std_reward': float(np.std(ep_rewards)), 'mean_violations': float(np.mean(ep_violations)), 'max_violations': int(np.max(ep_violations)), 'mean_length': float(np.mean(ep_lengths)), 'components': {k: float(np.mean(v)) for k, v in reward_components.items()}}
...
def compare_with_baseline(actor, env, safety, reward_fn, n_eval: int=50):
    ...
    results['random_policy'] = evaluate_policy(RandomActor(), env, SafetyGuard(), reward_fn, n_eval, deterministic=True)

    class FixedActor:

        def deterministic(self, s):
            return torch.tensor([[0.5, 0.5, 0.5, 0.5, 0.7, 0.8]])

        def parameters(self):
            return iter([torch.zeros(1)])
    results['fixed_moderate'] = evaluate_policy(FixedActor(), env, SafetyGuard(), reward_fn, n_eval, deterministic=True)
    return results
...
def main():
    ...
    with open(out_csv, 'w', newline='') as f:
        ...
        for name, m in comparison.items():
            w.writerow([name, f"{m['mean_reward']:.4f}", f"{m['std_reward']:.4f}", f"{m['mean_violations']:.1f}", '>random'])
