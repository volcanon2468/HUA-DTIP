...
def eval_rollout_consistency(engine: MCRolloutEngine, z0_mean: torch.Tensor,
                              z0_std: torch.Tensor, n_repeats: int = 5) -> dict:
    ...
    return {
        "consistency_std": float(hr_finals.std()),
        "consistency_mean": float(hr_finals.mean()),
        "consistency_cv": float(hr_finals.std() / (hr_finals.mean() + 1e-8)),
    }


def eval_risk_calibration(engine: MCRolloutEngine, vae: BayesianVAE,
                           z0: torch.Tensor) -> dict:
    with torch.no_grad():
    ...
    return {
        "high_intensity_overtraining_prob": overtraining_high,
        "low_intensity_overtraining_prob":  overtraining_low,
        "risk_discriminability": overtraining_high - overtraining_low,
    }
...
def main():
    ...
    with open(out_ranking, "w") as f:
        json.dump([{
            "name": r.name, "fitness_score": r.fitness_score,
            "overtraining_prob": r.overtraining_prob, "injury_prob": r.injury_prob,
            "peaking_day": r.peaking_day,
