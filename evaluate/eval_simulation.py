...
def eval_risk_calibration(engine: MCRolloutEngine, vae: BayesianVAE, z0_features: torch.Tensor) -> dict:
    with torch.no_grad():
        mu, logvar = vae.encoder(z0_features)
...
def main():
    ...
    z0_features = torch.randn(1, FEATURE_DIM, device=device)
    with torch.no_grad():
        mu, logvar = vae.encoder(z0_features)
        std = torch.exp(0.5 * logvar)
    ...
    for k, v in consistency.items():
        print(f'  {k}: {v:.4f}')
    print('\n=== Risk Calibration ===')
    risk_cal = eval_risk_calibration(mc_engine, vae, z0_features)
    for k, v in risk_cal.items():
        print(f'  {k}: {v:.4f}')
    print('\n=== Intervention Ranking ===')
    ranked = rank_interventions(what_if, z0_features)
