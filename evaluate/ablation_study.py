...
ABLATION_CONFIGS = {'full_model': {'remove': []}, 'no_imu_encoder': {'remove': ['imu']}, 'no_cardio_encoder': {'remove': ['cardio']}, 'no_feature_encoder': {'remove': ['feature']}, 'no_fusion': {'remove': ['fusion']}, 'no_tcn_only_transformer': {'remove': ['tcn']}, 'no_safety_guard': {'remove': ['safety']}, 'no_ewc': {'remove': ['ewc']}, 'beta_vae_beta_1': {'override': {'beta': 1.0}}, 'beta_vae_beta_8': {'override': {'beta': 8.0}}, 'sde_no_rest_context': {'remove': ['rest_context']}, 'no_fedper_personal': {'remove': ['fedper']}}
...
def run_ablation(checkpoint_dir: str, device, ablation_name: str, config: dict) -> dict:
    ...
    class NoSafetyWrapper:
        ...
        def check_and_clip(self, a, z, s):
            return a

        def compute_penalty(self, a, z, s):
            return 0.0
    ...
    return {'ablation': ablation_name, 'mean_reward': float(eval_results['mean_reward']), 'std_reward': float(eval_results['std_reward']), 'mean_violations': float(eval_results['mean_violations']), 'components': eval_results.get('components', {})}
