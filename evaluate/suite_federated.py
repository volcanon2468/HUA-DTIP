...
def run_federated_eval(processed_dir: str, checkpoint_dir: str, device) -> dict:
    ...
    return {'n_subjects_evaluated': n_subjects, 'avg_personalization_improvement_pct': float(avg_improvement), 'improvement_target_pct': 15.0, 'avg_cold_start_mae': float(avg_cold_mae), 'cold_start_warmup_samples': 20, 'per_subject_personalization': personal, 'per_subject_cold_start': cold}
if __name__ == '__main__':
    ...
    print(json.dumps({k: v for k, v in results.items() if k not in ('per_subject_personalization', 'per_subject_cold_start')}, indent=2))