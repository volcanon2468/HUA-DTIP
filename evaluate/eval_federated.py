...
def eval_personalization(processed_dir: str, checkpoint_dir: str, device: torch.device, n_personal_epochs: int=10) -> list:
    ...
    for sid in available:
        ...
        test_loader = DataLoader(SubjectDataset.__new__(SubjectDataset), batch_size=32, shuffle=False, collate_fn=_collate_fn)
        ...
        client = FedPerClient(personal_model, ['personal_head'], client_id=sid, lr=0.0005)
        ...
        results.append({'subject_id': sid, 'global_mae': global_mae, 'personalized_mae': personal_mae, 'improvement_pct': (global_mae - personal_mae) / (global_mae + 1e-08) * 100})
        print(f"  Subject {sid:3d}  global={global_mae:.4f}  personal={personal_mae:.4f}  Delta={results[-1]['improvement_pct']:.1f}%")
    return results

def eval_cold_start(processed_dir: str, checkpoint_dir: str, device: torch.device, n_warmup_samples: int=20) -> list:
    global_model = FederatedModel().to(device)
    p = os.path.join(checkpoint_dir, 'federated_global.pt')
    if os.path.exists(p):
        global_model.load_state_dict(torch.load(p, map_location=device))
    ...
    for sid in available:
        ...
        personal_model = copy.deepcopy(global_model)
        client = FedPerClient(personal_model, ['personal_head'], client_id=sid, lr=0.001)
        warmup_loader = DataLoader(warmup_ds, batch_size=min(16, n_warmup_samples), shuffle=True, collate_fn=_collate_fn)
        client.personalize(warmup_loader, nn.MSELoss(), device, n_epochs=5)
        test_loader = DataLoader(test_ds, batch_size=32, shuffle=False, collate_fn=_collate_fn)
        cold_mae = _evaluate_model(personal_model, test_loader, device)
        results.append({'subject_id': sid, 'warmup_samples': n_warmup_samples, 'cold_start_mae': cold_mae})
...
def main():
    ...
    with open(out_summary, 'w') as f:
        json.dump({'personalization': personal_results, 'cold_start': cold_results}, f, indent=2)
