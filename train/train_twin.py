...
class ZTemporalDataset(Dataset):
    ...
    def __init__(self, processed_dir: str, z_dim: int=512):
        self.paths = sorted(glob.glob(os.path.join(processed_dir, 'subject_*', 'windows', 'window_*.pt')))
        self.z_dim = z_dim
...
def _load_encoders_temporal(cfg, device):
    ...
    name_map = {'encoder_imu': enc_imu, 'encoder_cardio': enc_cardio, 'encoder_feature': enc_feat, 'encoder_fusion': enc_fusion, 'temporal_micro': micro, 'temporal_meso': meso, 'temporal_macro': macro, 'temporal_fusion': hier}
...
def train_vae(vae: BayesianVAE, loader: DataLoader, cfg: DictConfig, device: torch.device, feat_mean=None, feat_std=None):
    vae.train()
    opt = torch.optim.Adam(vae.parameters(), lr=cfg.training.twin.lr, weight_decay=cfg.training.twin.weight_decay)
    ...
    for epoch in range(cfg.training.twin.epochs):
        ...
        scheduler.step()
        log_metrics({'vae/loss': avg, 'vae/recon': parts['recon'], 'vae/kl': parts['kl'], 'vae/pred': parts['pred']}, step=epoch)
        if avg < best_loss:
            best_loss = avg
            patience_count = 0
        else:
...
def train_sde(sde: LatentNeuralSDE, vae: BayesianVAE, day_loader: DataLoader, cfg: DictConfig, device: torch.device):
    sde.train()
    vae.eval()
    ...
    for epoch in range(cfg.training.twin.sde.epochs):
        ...
        for x_seq, y_next in day_loader:
            ...
            z_pred = zs[-1]
            with torch.no_grad():
                y_input = torch.cat([y_next[:, :48], torch.zeros(y_next.shape[0], 512 - 48, device=device)], dim=-1)
...
def joint_finetune(vae: BayesianVAE, sde: LatentNeuralSDE, loader: DataLoader, day_loader: DataLoader, cfg: DictConfig, device: torch.device, feat_mean=None, feat_std=None):
    for p in list(vae.parameters()) + list(sde.parameters()):
        p.requires_grad = True
    opt = torch.optim.Adam(list(vae.parameters()) + list(sde.parameters()), lr=cfg.training.twin.joint_finetune_lr)
    mse = nn.MSELoss()
    for epoch in range(20):
        ...
        for (features, hrv, hr), (x_seq, y_next) in zip(loader, day_loader):
            ...
            z_inp = torch.cat([x_seq[:B, -1, :48], torch.zeros(B, 512 - 48, device=device)], dim=-1)
...
def main(cfg: DictConfig):
    ...
    feat_loader = DataLoader(feat_ds, batch_size=cfg.training.twin.batch_size, shuffle=True, num_workers=2, drop_last=True)
    ...
    feat_std = all_feats_cat.std(dim=0, keepdim=True).clamp(min=1e-06)
    torch.save({'mean': feat_mean, 'std': feat_std}, os.path.join('checkpoints', 'feature_norm_stats.pt'))
    ...
    joint_finetune(vae, sde, feat_loader, day_loader, cfg, device, feat_mean=feat_mean, feat_std=feat_std)
    log_model(vae, 'twin_vae', cfg)
    log_model(sde, 'twin_sde', cfg)
