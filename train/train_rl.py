...
class ReplayBuffer:
    def __init__(self, capacity: int = 100_000, state_dim: int = 20, action_dim: int = 6):
...
def _update(actor, critic, target_critic, log_alpha, target_entropy,
            opt_actor, opt_critic, opt_alpha,
            buffer, batch_size, gamma, tau, device):
...
def train_sac(cfg: DictConfig, device: torch.device):
    ...
    buffer     = ReplayBuffer(
        capacity=cfg.training.rl.buffer_size,
        state_dim=state_dim,
    ...
    for episode in range(cfg.training.rl.n_episodes):
        ...
        for step in range(env.episode_len):
            ...
            if buffer.size >= cfg.training.rl.warmup_steps:
                _update(
                    ...
                    opt_actor, opt_critic, opt_alpha,
                    buffer, cfg.training.rl.batch_size,
                    cfg.training.rl.gamma, cfg.training.rl.tau, device,
                )
            if done:
                break
        ...
        if episode % 100 == 0:
            print(
                f'  Episode {episode:5d}  reward={ep_reward:.3f}  '                f'avg100={avg_reward:.3f}  alpha={log_alpha.exp().item():.3f}  '                f'violations={safety.episode_violations}'
            )
