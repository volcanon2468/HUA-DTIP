...
class ReplayBuffer:
    ...
    def sample(self, batch_size: int, device: torch.device):
        ...
        return (torch.tensor(self.states[idx], device=device), torch.tensor(self.actions[idx], device=device), torch.tensor(self.rewards[idx], device=device).unsqueeze(-1), torch.tensor(self.next_states[idx], device=device), torch.tensor(self.dones[idx], device=device).unsqueeze(-1))
...
def train_sac(cfg: DictConfig, device: torch.device):
    ...
    for episode in range(cfg.training.rl.n_episodes):
        ...
        for step in range(env.episode_len):
            ...
            shaped_reward = reward_fn.compute(info['z_mu'].squeeze(), info['z_std'].squeeze(), action_np, prev_z_mu)
            ...
            if buffer.size >= cfg.training.rl.warmup_steps:
                _update(actor, critic, target_critic, log_alpha, target_entropy, opt_actor, opt_critic, opt_alpha, buffer, cfg.training.rl.batch_size, cfg.training.rl.gamma, cfg.training.rl.tau, device)
            if done:
                break
        ep_rewards.append(ep_reward)
        avg_reward = np.mean(ep_rewards)
        log_metrics({'rl/ep_reward': ep_reward, 'rl/avg_reward_100': avg_reward, 'rl/alpha': log_alpha.exp().item(), 'rl/safety_violations': safety.episode_violations}, step=episode)
        if avg_reward > best_avg and episode > 100:
            best_avg = avg_reward
            log_model(actor, 'rl_actor', cfg)
            log_model(critic, 'rl_critic', cfg)
        if episode % 100 == 0:
            print(f'  Episode {episode:5d}  reward={ep_reward:.3f}  avg100={avg_reward:.3f}  alpha={log_alpha.exp().item():.3f}  violations={safety.episode_violations}')
    return (actor, critic)

def _update(actor, critic, target_critic, log_alpha, target_entropy, opt_actor, opt_critic, opt_alpha, buffer, batch_size, gamma, tau, device):
