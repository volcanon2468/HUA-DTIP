...
class FederatedModel(nn.Module):
    def __init__(self):
        ...
        self.shared_encoder = nn.Sequential(
            nn.Linear(48, 128), nn.ReLU(inplace=True),
...
def simulate_federated(cfg: DictConfig, device: torch.device):
    ...
    for sid in selected_sids:
        ...
        client = FedPerClient(
            client_model, personal_layers, client_id=sid,
            lr=cfg.training.federated.client_lr,
            local_epochs=cfg.training.federated.local_epochs,
        )
    ...
    for rnd in range(n_rounds):
        ...
        clients_this_round = np.random.choice(
            len(clients), size=max(1, int(0.3 * len(clients))), replace=False
        )
        ...
        for ci in clients_this_round:
            ...
            shared_state = client.train_local(
                loader, loss_fn, device,
