...
def train_micro(micro_model, imu_enc, cardio_enc, feat_enc, fusion_mod,
                loader, cfg, device):
    micro_model.train()
    ...
    opt = torch.optim.Adam(micro_model.parameters(), lr=cfg.training.temporal.lr,
                           weight_decay=cfg.training.temporal.weight_decay)
    mse = nn.MSELoss()
    patience_count = 0; best_loss = float("inf")
    ...
    for epoch in range(cfg.training.temporal.epochs):
        ...
        for imu_seq, cardio_seq, feat_seq, hr_seq, hrv_seq in loader:
            ...
            loss = mse(hr_pred.squeeze(-1), hr_seq[:, -5:].mean(dim=1)) + \
                   mse(hrv_pred, hrv_seq[:, -5:, :].mean(dim=1))
...
def train_meso(meso_model, loader, cfg, device):
    ...
    for epoch in range(cfg.training.temporal.epochs):
        ...
        for x, y in loader:
            ...
            opt.zero_grad(); loss.backward(); opt.step()
            total_loss += loss.item() * x.shape[0]; n += x.shape[0]
